...
class FTNode extends d3.dagNode {
    ...
    union_label(union_label_func) {
            ...
            if (!union_label_func) {} else { this.union_label_func = union_label_func };
            return this;
        };
...
class Person extends FTNode {
    ...
    is_female() {
        if (this.data.gender !== undefined && this.data.gender === 'female') {
            return true;
        }
        return false;
    }
    ...
    is_divorced() {
        if (this.data.divorced !== undefined && this.data.divorced === 'true') {
            return true;
        }
        return false;
    }
    ...
    get_parent_unions() {
        let unions = [this.data.parent_union]
            .map(id => this.ft_datahandler.find_node_by_id(id))
            .filter(node => node != undefined);
        let u_id = this.data.parent_union;
...
class FTDrawer {
    ...
    static default_tooltip_func(node) {
        ...
        let content =   ``
        /*if (node.get_pic() !== undefined) {
             content = content + `
                <img src="1a.jpg" style="border-radius: 50%; max-width:128px; max-height:128px;"  alt="picture unavailable"/>
            `
        }*/
         content = content + `
                                <table style="margin-top: 2.5px;">
                                <tr>
                                    <tr><td colspan="2"><b>` + node.get_name() + `</b></td></tr>
                                </tr>
                                `


        if (node.get_birth_year() !== undefined) {
            content = content + `
                                <tr>
                                    <td>born</td>
                                    <td>` + (node.get_birth_year() || "?") + ` in ` + (node.data.birthplace || "?") + `</td>
                                </tr>
                                `
        }

        if (node.get_death_year() !== undefined) {
            content = content + `
                                <tr>
                                    <td>died</td>
                                    <td>` + (node.get_death_year() || "?") + ` in ` + (node.data.deathplace || "?") + `</td>
                                </tr>
                                `
        }

        if (node.get_summary() !== undefined) {
            content = content + `
                                            <tr><td colspan="2"><hr></td></tr>
                                            <tr>
                                            <td colspan="2">
                                 `
            node.get_summary().map((x) => content = (content + `<li>`+ x +`</li>`));
    ...
    static default_node_label_func(node) {
        ...
        if (node.get_birth_year()) {
            displayLabel = node.get_birth_year() + " born"
        }
    ...
    static default_node_class_func(node) {
        ...
        if (node.is_union()) return;
            ...
            if (node.is_extendable()) {
                if (node.is_female()) {
                    if (node.is_divorced()) {
                        return "person female divorced extendable"
                    }
                    return "person female extendable"
                }
                if (node.is_divorced()) {
                    return "person divorced extendable"
                }
                return "person extendable"
            } else {
                if (node.is_female()) {
                   if (node.is_divorced()) {
                        return "person female divorced non-extendable"
                   }
                   return "person female non-extendable"
                }
               if (node.is_divorced()) {
                    return "person divorced non-extendable"
               }
